...
$('#Weather').html("<div class='row collapse'><span class='small-4 columns left'><img id='wIcon' src='"+ IconURL +"' /><h6 id='temp'>" + temperF +
"F&deg;<small>("+ temperC +"C&deg;)</small></h6></span><span class='small-5 columns left'><h6>Humidity: <b id='humid'>"+ humidity +"</b> <strong>| "
+ Wreport +".</strong></h6></span><small id='time' class='right'>"+ time +"</small></div>");
   }
  });
}
...
$('.notice').fadeOut(5000);
    ...
    $(document).on("dblclick", "#delTable tbody td span", function( ){
	$(this).designMode = 'on';
  var editable = $(this).attr('contentEditable','true').text();
    if (localStorage.getItem('contenteditable')) {
      $(this).html(localStorage.getItem('contenteditable') );
      $('#save').fadeIn(800);     
      $('#cancel').fadeIn(800);
    }		  
    $(this).css({'background':'#ffffff','color':'#FF0000','border':'1px solid #FF0000' });
	      var curVal = $(this).text();
          var parent= $(this).parent();
		  var ticketNumber = $(this).parent('td').parent('tbody tr').attr('id');
		  $(this).attr('contentEditable','true');
              $(this).parent('td').css({'background': '#FFCCCC', 'color': '#000000', 'border' : '2px double #000000', 'padding' : '5px'});      
		  console.log(curVal);
		    $(this).blur( function() {		
			    var message = "Please remember to save your changes!";
                  document.designMode = 'off';
		          var newVal = $(this).text();
                  if( curVal != $(this).text()) {  				       $('#save').fadeIn(800);
					   addNotice('<p>'+ message +'</p>');
                       console.log(message); 
				       $('#cancel').fadeIn(800);
	                } else if(curVal == $(this).text()){
				       $(this).parent('td').css({'background': '#FFFFCC', 'color': '#FF0000', 'border' : '1px solid #000000'});
                       console.log(curVal +' did not change.');
                       }
			   $(document).on("click", "#save", function(editable) {
			   var message = 'Value "'+ curVal +'" becomes new value "'+ newVal +'" for ticket #'+ ticketNumber +'. modified by '+ $USER;
                   localStorage.setItem('contenteditable', newVal);
				   addNotice('<p>'+ message +'</p>');
                });
