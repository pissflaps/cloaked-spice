...
Print("<THEAD><TR><TH>Filename</TH><th>Filesize</th><th>Last_Modified</th><th>Remove File?</th></TR></THEAD>\n");
Print ("<TBODY>\n");
	  ...
	  $filetime = filemtime($dirArray[$index]);
	   $last_mod = date ("m/d/Y H:i:s", $filetime);
        ...
        if (substr("$dirArray[$index]", 0, 1) != "."){ // don't list hidden files
	        print("<TR><TD class='linkText'><a href='$filePath' TARGET='_NEW'>$fileName</a></td>");
...
print("</TBODY></TABLE>\n");
print ("<h2 class='right'> $indexCount total files found.</h2> \n");
?>
  </div>
...
function deleteFile(id){
    $.ajax({
		...
		data: 'fileid=' +id,
        success: function() {
            $('#lead').html('<h2>'+ id +' was deleted successfully. Click OK to reload the page.</h2> <br> <button class="large button success expand" id="RELOAD">OK</button>');
...
$(document).ready(function() {
    ...
    $('.DELETEME').on('click',function(){
	    $ID = $(this).attr('id');
		console.log($ID);
		$buttonHTML = '<button class="medium button primary" id="REMOVE">Do it! </button>  <button class="medium button alert" id="CANCEL">CANCEL </button>';
		$('#lead').html("<h3 class='subheader'>Delete <a href='http://tac-alert01/tmp/"+$ID+"' target='_NEW'> " +$ID+ "</a> from the Alert System?</h3> <BR>" +$buttonHTML);
