...
include('autologin.php');
  ...
  $Username = mysql_query("select username from dbase.members where IP='$IP'");
  $User_found = mysql_fetch_row($Username);
  if (isset($User_found) ){
    $Log_in = false;
	} else { 
	  $Log_in = true;
...
<?php	
 $IP = $_SERVER['REMOTE_ADDR'];
  ...
  var $MyLogin = $checkup[0];
  var $MyPass = $checkup[1];
  ?>
...
</div><span id="RegUser"><a href="RegUsr.php">Click here <br> to register.</a></span>
<?php
};
